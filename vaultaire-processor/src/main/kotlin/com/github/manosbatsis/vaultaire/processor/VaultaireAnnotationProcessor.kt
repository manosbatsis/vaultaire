...
class VaultaireAnnotationProcessor : AbstractProcessor() {
    ...
    fun writeBuilder(persistentStateTypeElement: TypeElement, contractStateTypeElement: Element, fields: List<VariableElement>, sourceRoot: File) {
        ...
        val generatedFieldsClassName = ClassName(stateInfo.generatedPackageName, "${stateInfo.persistentStateSimpleName}Fields")
        ...
        val fieldsSpec = buildFieldsObjectSpec(stateInfo, generatedFieldsClassName)
        ...
        FileSpec.builder(stateInfo.generatedPackageName, "${contractStateTypeElement.simpleName}VaultaireGenerated")
    ...
    fun buildFieldsObjectSpec(stateInfo: StateInfo, generatedFieldsClassName: ClassName): TypeSpec.Builder {
        val fieldsSpec = TypeSpec.objectBuilder(generatedFieldsClassName)
                .addSuperinterface(FIELDS_CLASSNAME.parameterizedBy(stateInfo.persistentStateTypeElement.asKotlinTypeName()))
                .addKdoc("Provides easy access to fields of [%T]", stateInfo.persistentStateTypeElement)
        // Note fields by name
        ...
        stateInfo.fields.forEachIndexed { index, field ->
            val fieldProperty = buildPersistentStateFieldWrapperPropertySpec(field, stateInfo.persistentStateTypeElement)
            fieldsSpec.addProperty(fieldProperty)
            fieldsByNameBuilder.addStatement("\t%S to ${fieldProperty.name} ${if (index + 1 < stateInfo.fields.size) "," else ""}", fieldProperty.name)
        }
